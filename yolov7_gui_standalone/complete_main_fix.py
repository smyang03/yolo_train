"""
main.py와 UI 파일을 Professional GUI로 완전 교체
"""

def fix_main_and_ui():
    """main.py와 UI 파일을 완전히 수정"""
    
    # 1. 새로운 main.py 코드 (간단한 UI 생성 부분 제거)
    new_main_code = '''"""
...
'''
    ...
    # 2. Professional main_window.py 코드 (완전한 버전)
    professional_ui = '''"""
...
class MainWindow:
    ...
    def create_header(self):
        ...
        title_label = tk.Label(header_frame, text="🚀 YOLOv7 Professional Training GUI", 
                              font=('Arial', 20, 'bold'), fg='white', bg='#2c3e50')
        title_label.pack(expand=True)
        
        subtitle_label = tk.Label(header_frame, text="Advanced Object Detection Model Training Platform",
    ...
    def create_dataset_section(self, parent):
        ...
        # 데이터셋 경로
        ttk.Label(dataset_frame, text="Dataset Path (data.yaml):", font=('Arial', 11, 'bold')).pack(anchor='w')
        dataset_path_frame = ttk.Frame(dataset_frame)
        dataset_path_frame.pack(fill='x', pady=5)
        
        dataset_entry = ttk.Entry(dataset_path_frame, textvariable=self.dataset_path_var, font=('Arial', 10), width=70)
        ...
        # 모델 설정
        ttk.Label(dataset_frame, text="Model Configuration:", font=('Arial', 11, 'bold')).pack(anchor='w', pady=(15, 0))
        model_combo = ttk.Combobox(dataset_frame, textvariable=self.model_config_var, font=('Arial', 10), width=70)
        model_combo['values'] = [
        ...
        # 사전 훈련된 가중치
        ttk.Label(dataset_frame, text="Pretrained Weights (선택사항):", font=('Arial', 11, 'bold')).pack(anchor='w', pady=(15, 0))
        weights_frame = ttk.Frame(dataset_frame)
        weights_frame.pack(fill='x', pady=5)
        
        weights_entry = ttk.Entry(weights_frame, textvariable=self.weights_path_var, font=('Arial', 10), width=70)
        ...
        # 이미지 크기
        ttk.Label(dataset_frame, text="Image Size:", font=('Arial', 11, 'bold')).pack(anchor='w', pady=(15, 0))
        size_combo = ttk.Combobox(dataset_frame, textvariable=self.image_size_var, font=('Arial', 10), width=70)
    ...
    def create_training_params_section(self, parent):
        ...
        # Epochs
        ttk.Label(params_grid, text="Epochs:", font=('Arial', 11, 'bold')).grid(row=0, column=0, sticky='w', padx=(0, 20))
        ...
        epochs_scale = ttk.Scale(epochs_frame, from_=1, to=1000, variable=self.epochs_var, 
                               orient='horizontal', length=300, command=self.update_epochs_label)
        ...
        # Batch Size
        ttk.Label(params_grid, text="Batch Size:", font=('Arial', 11, 'bold')).grid(row=1, column=0, sticky='w', padx=(0, 20), pady=(15, 0))
        batch_frame = ttk.Frame(params_grid)
        batch_frame.grid(row=1, column=1, sticky='ew', pady=(15, 5))
        
        batch_scale = ttk.Scale(batch_frame, from_=1, to=64, variable=self.batch_size_var,
                              orient='horizontal', length=300, command=self.update_batch_size_label)
        batch_scale.pack(side='left')
        self.batch_size_label = ttk.Label(batch_frame, text="16", font=('Arial', 11, 'bold'))
        self.batch_size_label.pack(side='left', padx=(10, 0))
        
        # Learning Rate
        ttk.Label(params_grid, text="Learning Rate:", font=('Arial', 11, 'bold')).grid(row=2, column=0, sticky='w', padx=(0, 20), pady=(15, 0))
        lr_frame = ttk.Frame(params_grid)
        lr_frame.grid(row=2, column=1, sticky='ew', pady=(15, 5))
        
        lr_scale = ttk.Scale(lr_frame, from_=0.001, to=0.1, variable=self.learning_rate_var,
                           orient='horizontal', length=300, command=self.update_lr_label)
        lr_scale.pack(side='left')
        self.lr_label = ttk.Label(lr_frame, text="0.01", font=('Arial', 11, 'bold'))
        self.lr_label.pack(side='left', padx=(10, 0))
        
        # Workers
        ttk.Label(params_grid, text="Workers:", font=('Arial', 11, 'bold')).grid(row=3, column=0, sticky='w', padx=(0, 20), pady=(15, 0))
        workers_frame = ttk.Frame(params_grid)
        workers_frame.grid(row=3, column=1, sticky='ew', pady=(15, 5))
        
        workers_scale = ttk.Scale(workers_frame, from_=0, to=16, variable=self.workers_var,
                                 orient='horizontal', length=300, command=self.update_workers_label)
        workers_scale.pack(side='left')
        self.workers_label = ttk.Label(workers_frame, text="8", font=('Arial', 11, 'bold'))
        self.workers_label.pack(side='left', padx=(10, 0))
        
        # Device
        ttk.Label(params_grid, text="Device:", font=('Arial', 11, 'bold')).grid(row=4, column=0, sticky='w', padx=(0, 20), pady=(15, 0))
        device_combo = ttk.Combobox(params_grid, textvariable=self.device_var, values=["0", "0,1", "0,1,2,3", "cpu"], width=20)
        device_combo.grid(row=4, column=1, sticky='w', pady=(15, 5))
    ...
    def create_options_section(self, parent):
        ...
        output_frame.pack(fill='x', pady=10, padx=15)
        
        ttk.Label(output_frame, text="Experiment Name:", font=('Arial', 11, 'bold')).pack(anchor='w')
        ttk.Entry(output_frame, textvariable=self.experiment_name_var, font=('Arial', 10), width=50).pack(fill='x', pady=5)
    ...
    def create_progress_tab(self):
        ...
        # 상태 표시
        status_frame = ttk.LabelFrame(progress_frame, text="📊 Training Status", padding=15)
        ...
        self.status_label = ttk.Label(status_indicator_frame, textvariable=self.status_text_var, font=('Arial', 14, 'bold'))
        ...
        # 현재 메트릭 표시
        metrics_frame = ttk.LabelFrame(progress_frame, text="📈 Current Metrics", padding=15)
        metrics_frame.pack(fill='x', pady=10, padx=15)
        ...
        # 훈련 로그
        log_frame = ttk.LabelFrame(progress_frame, text="📝 Training Log", padding=15)
        log_frame.pack(fill='both', expand=True, pady=10, padx=15)
        ...
        # 초기 로그 메시지
        self.add_log_entry("🎉 YOLOv7 Professional GUI가 성공적으로 시작되었습니다!")
        self.add_log_entry("📁 YOLOv7 경로: " + str(self.trainer.yolo_original_dir))
        self.add_log_entry("💡 완전한 기능을 갖춘 전문적인 훈련 인터페이스입니다.")
        self.add_log_entry("⚙️ 설정 탭에서 데이터셋과 파라미터를 조정하세요.")
    ...
    def create_metric_displays(self, parent):
        ...
        for i, (label_text, var_name, default_value) in enumerate(metrics):
            ...
            metric_frame = ttk.Frame(parent, relief='ridge', borderwidth=2)
            metric_frame.grid(row=row, column=col, sticky='ew', padx=5, pady=5)
            
            ttk.Label(metric_frame, text=label_text, font=('Arial', 10, 'bold')).pack(pady=(5, 0))
            
            value_label = ttk.Label(metric_frame, text=default_value, font=('Arial', 14, 'bold'), 
                                   foreground='#3498db')
    ...
    def create_results_tab(self):
        ...
        if MATPLOTLIB_AVAILABLE:
            # 차트 프레임
            charts_frame = ttk.LabelFrame(results_frame, text="📊 Performance Charts", padding=15)
            charts_frame.pack(fill='both', expand=True, pady=10, padx=15)
            
            # Matplotlib 차트
            self.create_charts(charts_frame)
        else:
            # Matplotlib이 없는 경우 대체 UI
            placeholder_frame = ttk.LabelFrame(results_frame, text="📊 Results Summary", padding=15)
            placeholder_frame.pack(fill='both', expand=True, pady=10, padx=15)
    ...
    def create_control_buttons(self):
        ...
        self.start_btn = ttk.Button(center_frame, text="🚀 Start Training", 
                                   command=self.start_training, width=15)
    ...
    def on_training_started(self, data):
        ...
        self.add_log_entry(f"🎯 Dataset: {Path(config.get('dataset_path', 'N/A')).name}")
    ...
    def on_training_stopped(self, data):
        """훈련 정지 콜백"""
        self.is_training = False
        self.status_text_var.set("⏹️ 훈련 정지됨")
        self.status_canvas.itemconfig(self.status_dot, fill='red')
    ...
    def update_charts(self):
        """차트 업데이트"""
        if not MATPLOTLIB_AVAILABLE or len(self.chart_data['epochs']) < 2:
            return
...
'''
    ...
    print("🔄 main.py와 UI 파일을 Professional GUI로 교체 중...")
    
    # 1. main.py 백업 및 교체
    main_file = Path("main.py")
    if main_file.exists():
        shutil.copy(main_file, "main_simple.py.backup")
        print("✅ 기존 main.py를 main_simple.py.backup으로 백업")
    
    with open(main_file, 'w', encoding='utf-8') as f:
        f.write(new_main_code)
    print("✅ main.py를 Professional 버전으로 교체")
    
    # 2. main_window.py 백업 및 교체
    ui_file = Path("src/ui/main_window.py")
    if ui_file.exists():
        shutil.copy(ui_file, "src/ui/main_window_simple.py.backup")
        print("✅ 기존 main_window.py를 backup으로 백업")
    
    with open(ui_file, 'w', encoding='utf-8') as f:
        f.write(professional_ui)
    print("✅ main_window.py를 Professional 버전으로 교체")
    
    print("\\n🎉 Professional GUI로 완전 교체 완료!")
    print("\\n📝 변경 사항:")
    print("   ✅ main.py - 간단한 UI 생성 부분 제거")
    print("   ✅ main_window.py - 완전한 Professional GUI")
    print("   ✅ 3탭 인터페이스 (설정/진행사항/결과)")
    print("   ✅ 고급 파라미터 설정")
    print("   ✅ 실시간 메트릭 표시")
    print("   ✅ 성능 차트 (Matplotlib)")
    print("   ✅ 전문적인 디자인")
    print("\\n🚀 사용법:")
    print("   1. 현재 실행 중인 GUI 종료")
    print("   2. python main.py 다시 실행")
    print("   3. Professional GUI 확인!")
