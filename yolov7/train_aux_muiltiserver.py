...
if __name__ == '__main__':
    ...
    if opt.local_rank != -1:
        ...
        if not dist.is_initialized():
            # env:// 대신 명시적인 TCP 주소 사용
            master_addr = os.environ.get('MASTER_ADDR', '192.168.79.241')
            master_port = os.environ.get('MASTER_PORT', '9527')
            
            # 더 긴 타임아웃 설정
            timeout = datetime.timedelta(seconds=1800)

            rank = int(os.environ.get('RANK', opt.local_rank))  # RANK 환경 변수 또는 local_rank 사용
            world_size = int(os.environ.get('WORLD_SIZE', opt.world_size))  # WORLD_SIZE 환경 변수 또는 world_size 사용
            ...
            print(f'[{opt.local_rank}] Process group initialized with {master_addr}:{master_port}, rank={rank}, world_size={world_size}')
